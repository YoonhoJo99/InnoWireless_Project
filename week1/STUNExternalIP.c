#include "STUNExternalIP.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <sys/socket.h>
#include <netdb.h>

struct STUNMessageHeader {
    unsigned short type;
    unsigned short length;
    unsigned int cookie;
    unsigned int identifier[3];
};

#define XOR_MAPPED_ADDRESS_TYPE 0x0020

struct STUNAttributeHeader {
    unsigned short type;
    unsigned short length;
};

struct STUNXORMappedIPv4Address {
    unsigned char reserved;
    unsigned char family;
    unsigned short port;
    unsigned int address;
};

int getPublicIPAddressAndPort(struct STUNServer server, char* address, unsigned short* port) {
    int socketd = socket(AF_INET, SOCK_STREAM, 0);
    if (socketd < 0) {
        perror("Socket creation failed");
        return -1;
    }

    struct sockaddr_in localAddress;
    memset(&localAddress, 0, sizeof(localAddress));
    localAddress.sin_family = AF_INET;
    localAddress.sin_addr.s_addr = INADDR_ANY;
    localAddress.sin_port = htons(10000);  // 포트를 고정

    if (bind(socketd, (struct sockaddr*) &localAddress, sizeof(localAddress)) < 0) {
       perror("Socket bind failed");
        close(socketd);
        return -1;
    }

    struct addrinfo hints, *res;
    memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_INET;
    hints.ai_socktype = SOCK_STREAM;

    if (getaddrinfo(server.address, NULL, &hints, &res) != 0) {
        perror("Failed to resolve STUN server address");
        close(socketd);
        return -2;
    }

    struct sockaddr_in remoteAddress;
    memcpy(&remoteAddress, res->ai_addr, res->ai_addrlen);
    remoteAddress.sin_port = htons(server.port);

    if (connect(socketd, (struct sockaddr*) &remoteAddress, sizeof(remoteAddress)) < 0) {
        perror("TCP connection to STUN server failed");
        freeaddrinfo(res);
        close(socketd);
        return -3;
    }

    struct STUNMessageHeader request;
    request.type = htons(0x0001);
    request.length = htons(0x0000);
    request.cookie = htonl(0x2112A442);
    for (int i = 0; i < 3; i++) {
        request.identifier[i] = rand();
    }

    if (send(socketd, &request, sizeof(request), 0) < 0) {
        perror("STUN request send failed");
        freeaddrinfo(res);
        close(socketd);
        return -4;
    }

    char buffer[512];
    ssize_t length = recv(socketd, buffer, sizeof(buffer), 0);
    if (length < 0) {
        perror("STUN response receive failed");
        freeaddrinfo(res);
        close(socketd);
        return -5;
    }

    struct STUNMessageHeader* response = (struct STUNMessageHeader*) buffer;
    if (response->type == htons(0x0101)) {
        char* pointer = buffer + sizeof(struct STUNMessageHeader);
        while (pointer < buffer + length) {
            struct STUNAttributeHeader* header = (struct STUNAttributeHeader*) pointer;
            if (header->type == htons(XOR_MAPPED_ADDRESS_TYPE)) {
                pointer += sizeof(struct STUNAttributeHeader);
                struct STUNXORMappedIPv4Address* xorAddress = (struct STUNXORMappedIPv4Address*) pointer;

                *port = ntohs(xorAddress->port) ^ 0x2112;
                unsigned int numAddress = xorAddress->address ^ htonl(0x2112A442);
                snprintf(address, 20, "%d.%d.%d.%d",
                         (numAddress >> 24) & 0xFF,
                         (numAddress >> 16) & 0xFF,
                         (numAddress >> 8) & 0xFF,
                         numAddress & 0xFF);

                freeaddrinfo(res);
                close(socketd);
                return 0;
            }
            pointer += sizeof(struct STUNAttributeHeader) + ntohs(header->length);
        }
    }

    freeaddrinfo(res);
    close(socketd);
    return -6;
}

